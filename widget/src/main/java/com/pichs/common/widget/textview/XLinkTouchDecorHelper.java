...
public class XLinkTouchDecorHelper {
    ...
    public boolean onTouchEvent(TextView textView, Spannable spannable, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (textView instanceof XISpanTouch) {
                XISpanTouch tv = (XISpanTouch) textView;
                tv.setTouchSpanHit(mPressedSpan != null);
            }
            return mPressedSpan != null;
        } else if (event.getAction() == MotionEvent.ACTION_MOVE) {
            XITouchableSpan touchedSpan = getPressedSpan(textView, spannable, event);
    ...
    public XITouchableSpan getPressedSpan(TextView textView, Spannable spannable, MotionEvent event) {
        ...
        try {
            ...
            XITouchableSpan[] link = spannable.getSpans(off, off, XITouchableSpan.class);
            XITouchableSpan touchedSpan = null;
            if (link.length > 0) {
                touchedSpan = link[0];
            }
            return touchedSpan;
