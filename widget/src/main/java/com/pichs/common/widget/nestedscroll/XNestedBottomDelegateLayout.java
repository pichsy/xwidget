...
import com.pichs.common.widget.utils.XViewHelper;
...
public abstract class XNestedBottomDelegateLayout extends XCardFrameLayout implements NestedScrollingChild2, NestedScrollingParent2, XNestedBottomView {
    public static final String KEY_SCROLL_INFO_OFFSET = "@xwidget_scroll_info_bottom_dl_offset";

    private final NestedScrollingParentHelper mParentHelper;
    private final NestedScrollingChildHelper mChildHelper;
    private View mHeaderView;
    private View mContentView;
    private XViewOffsetHelper mHeaderViewOffsetHelper;
    private XViewOffsetHelper mContentViewOffsetHelper;
    ...
    public XNestedBottomDelegateLayout(Context context) {
        this(context, null);
    }

    public XNestedBottomDelegateLayout(Context context, AttributeSet attrs) {
        this(context, null, 0);
    }

    public XNestedBottomDelegateLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (!(mContentView instanceof XNestedBottomView)) {
        ...
        mHeaderViewOffsetHelper = new XViewOffsetHelper(mHeaderView);
        mContentViewOffsetHelper = new XViewOffsetHelper(mContentView);
    ...
    private int getMiniOffset() {
        XNestedBottomView b = (XNestedBottomView) mContentView;
    ...
    public int getContentHeight() {
        ...
        if (bc == XNestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL || bc > mContentView.getHeight()) {
            return XNestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL;
        }
    ...
    public void checkLayout() {
        ...
        XNestedBottomView bottomView = (XNestedBottomView) mContentView;
    ...
    private int offsetBy(int dyUnConsumed) {
        ...
        mOnScrollNotifier.notify(-mHeaderViewOffsetHelper.getTopAndBottomOffset(),
                mHeaderView.getHeight() + ((XNestedBottomView) mContentView).getScrollOffsetRange());
    ...
    public void consumeScroll(int dy) {
        if (dy == Integer.MAX_VALUE) {
            offsetBy(dy);
            ((XNestedBottomView) mContentView).consumeScroll(Integer.MAX_VALUE);
            return;
        } else if (dy == Integer.MIN_VALUE) {
            ((XNestedBottomView) mContentView).consumeScroll(Integer.MIN_VALUE);
            offsetBy(dy);
            return;
        }
        ((XNestedBottomView) mContentView).consumeScroll(dy);
    }

    @Override
    public void smoothScrollYBy(int dy, int duration) {
        ((XNestedBottomView) mContentView).smoothScrollYBy(dy, duration);
    }

    @Override
    public void stopScroll() {
        ((XNestedBottomView) mContentView).stopScroll();
    }

    @Override
    public int getCurrentScroll() {
        return -mHeaderViewOffsetHelper.getTopAndBottomOffset() +
                ((XNestedBottomView) mContentView).getCurrentScroll();
    }
    ...
    public int getScrollOffsetRange() {
        ...
        return mHeaderView.getHeight() - getHeaderStickyHeight() +
                ((XNestedBottomView) mContentView).getScrollOffsetRange();
    }
    ...
    public void injectScrollNotifier(final OnScrollNotifier notifier) {
        ...
        if (mContentView instanceof XNestedBottomView) {
            ((XNestedBottomView) mContentView).injectScrollNotifier(new OnScrollNotifier() {
                @Override
    ...
    public void saveScrollInfo(@NonNull Bundle bundle) {
        ...
        if (mContentView != null) {
            ((XNestedBottomView) mContentView).saveScrollInfo(bundle);
        }
    }
    ...
    public void restoreScrollInfo(@NonNull Bundle bundle) {
        ...
        offset = XLangHelper.constrain(offset, getMiniOffset(), 0);
        mHeaderViewOffsetHelper.setTopAndBottomOffset(offset);
        mContentViewOffsetHelper.setTopAndBottomOffset(offset);
        if (mContentView != null) {
            ((XNestedBottomView) mContentView).restoreScrollInfo(bundle);
        }
    }
    ...
    private boolean isPointInHeaderBounds(int x, int y) {
       XViewHelper.getDescendantRect(this, mHeaderView, mTempRect);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (isBeingDragged) {
                    lastMotionY = y;
                    if (dy < 0 && ((XNestedBottomView) mContentView).getCurrentScroll() > 0) {
    ...
    class ViewFlinger implements Runnable {
        ...
        Interpolator mInterpolator = XInterpolators.quintic;
        ...
        ViewFlinger() {
            mOverScroller = new OverScroller(getContext(),  XInterpolators.quintic);
        }
        ...
        private void internalPostOnAnimation() {
            ...
            ViewCompat.postOnAnimation(XNestedBottomDelegateLayout.this, this);
        ...
        public void fling(int velocityY) {
            ...
            if (mInterpolator !=  XInterpolators.quintic) {
                mInterpolator =  XInterpolators.quintic;
                mOverScroller = new OverScroller(getContext(),  XInterpolators.quintic);
            }
